...
**I also have a python-based command-line interface tool for UUP Dump currently in development, so that with a single command you can download the latest update package for a given channel, and run the scripts to generate an installable ISO for use with this tool. You can check out the project [here](https://github.com/JosephM101/uupdump_cli#one-line-run)**
